...
def load_or_get_user_input():
    ...
    if os.path.exists(config_file):
        with open(config_file, 'r') as f:
            config = json.load(f)
        return config['authorization'], config['min_balance_threshold']
    else:
        ...
        with open(config_file, 'w') as f:
            json.dump(config, f)
        return authorization, min_balance_threshold
...
# Main program loop
while True:
    authorization, min_balance_threshold = load_or_get_user_input()
    ...
    print(f"{Colors.YELLOW}Program completed. Waiting for 4 hours before restarting...{Colors.RESET}")
    sleep(4 * 60 * 60)  # 4 saat bekle